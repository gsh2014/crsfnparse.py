...
class Parser(nn.Module):
    ...
    # TODO: GSH NOT TESTED ENCODE
    def encode(self, src_sents_var, src_sents_len, is_lay_encoder=True):
        ...
        Args:
            ...
            src_sents_len: a list of lengths of input source sentences or , sorted by descending order
            is_lay_encoder: a boolean,encode the src sents if the value is false or encode the lay 
        Returns:
        ...
        # TODO:by gsh lay sents should pass the dropout layer,maybe ? 
        if self.training and self.args.word_dropout and not is_lay_encoder:
        ...
        packed_src_token_embed = pack_padded_sequence(token_embed, src_sents_len)

        # src_encodings: (tgt_query_len, batch_size, hidden_size)
        if not is_lay_encoder:
    ...
    def laydecode(self, batch, src_encodings, dec_init_vec):
        """Given a batch of examples and their encodings of input utterances,
        compute query vectors at each decoding time step, which are used to compute
        action probabilities
        ...
        Returns:
            ...
            Also return the attention weights over candidate tokens if using supervised attention
        """
        # TODO:gsh lay decoder arguments should be added
        batch_size = len(batch)
        args = self.args
        ...
        for t in range(batch.max_lay_action_num):
            # the input to the decoder LSTM is a concatenation of multiple signals
            # [
            #   embedding of previous action -> `a_tm1_embed`,
            #   previous attentional vector -> `att_tm1`,
            ...
            if t == 0:
                ...
                a_tm1_embeds = []
                for actions in batch.lay_actions:
                    # action t - 1
                    if t < len(actions):
                        a_tm1 = actions[t - 1]
                        if isinstance(a_tm1.action, ApplyRuleAction):
                ...
                if args.lay_no_parent_field_embed is False:
                    ...
                    inputs.append(parent_field_embed)
                if args.lay_no_parent_field_type_embed is False:
                    parent_field_type_embed = self.type_embed(batch.get_frontier_field_type_idx_lay(t))
                    inputs.append(parent_field_type_embed)

                # append history states
                actions_t = [actions[t] if t < len(actions) else None for actions in batch.lay_actions]
